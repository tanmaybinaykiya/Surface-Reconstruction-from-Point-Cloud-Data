...
class Points // class for manipulaitng and displaying pointclouds or polyloops in 3D 
  ...
  Points addPt(Point P, char c) { G[nv].setTo(P); pv=nv; L[nv]=c; nv++;  return this;}          // appends a new point at the end
  Points addPt(Point P) { G[nv].setTo(P); pv=nv; L[nv]='f'; nv++;  return this;}          // appends a new point at the end
  Points addPt(float x,float y) { G[nv].x=x; G[nv].y=y; pv=nv; nv++; return this;} // same byt from coordinates
  Points copyFrom(Points Q) {empty(); nv=Q.nv; for (int v=0; v<nv; v++) G[v]=P(Q.G[v]); return this;} // set THIS as a clone of Q

  Points perturb(float e) { for (int i=1; i<nv; i++) {G[i].x+=random(-e,e); G[i].y+=random(-e,e);} return this;}
  ...
  Points setPt(Point P, int i) { G[i].setTo(P); return this;}
  Points setZ(float z) {for (int v=0; v<nv; v++)  G[v].z=z; return this;}
  
  Points drawBalls(float r) {for (int v=0; v<nv; v++) show(G[v],r); return this;}
  Points showPicked(float r) {show(G[pv],r); return this;}
  ...
  Points movePicked(Vector V) { G[pv].add(V); return this;}      // moves selected point (index p) by amount mouse moved recently
  Points setPickedTo(Point Q) { G[pv].setTo(Q); return this;}      // moves selected point (index p) by amount mouse moved recently
  Points moveAll(Vector V) {for (int i=0; i<nv; i++) G[i].add(V); return this;};   
  Point Picked() {return G[pv];} 
  Point Pt(int i) {if(0<=i && i<nv) return G[i]; else return G[0];} 
